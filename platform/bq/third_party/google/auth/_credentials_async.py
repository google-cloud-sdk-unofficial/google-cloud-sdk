...
class Credentials(credentials.Credentials, metaclass=abc.ABCMeta):
    ...
    async def before_request(self, request, method, url, headers):  # pytype: disable=signature-mismatch
        ...
        if not self.valid:
            if inspect.iscoroutinefunction(self.refresh):  # pytype: disable=wrong-arg-types
                await self.refresh(request)
            else:
                self.refresh(request)

        if inspect.iscoroutinefunction(self._after_refresh):
            await self._after_refresh(request, method, url, headers)  # pytype: disable=bad-return-type
        else:
...
class AnonymousCredentials(credentials.AnonymousCredentials, Credentials):  # pytype: disable=signature-mismatch
...
class CredentialsWithRegionalAccessBoundary(
    ...
    async def _after_refresh(self, request, method, url, headers):  # pytype: disable=signature-mismatch
    ...
    async def _lookup_regional_access_boundary(self, request, fail_fast=False):  # pytype: disable=signature-mismatch
