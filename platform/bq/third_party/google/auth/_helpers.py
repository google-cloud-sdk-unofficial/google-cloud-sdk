...
_IS_HERMETIC_TEST = (
    "TEST_TMPDIR" in os.environ
    and not os.getenv("TEST_TARGET", "").startswith(
        "//third_party/py/google/auth"
    )
    and not os.getenv("TEST_TARGET", "").startswith("//corp/cloud/gae4g")
    and "GCE_METADATA_HOST" not in os.environ
    and "GCE_METADATA_ROOT" not in os.environ
    and "GCE_METADATA_IP" not in os.environ
)


def is_hermetic_test():
  """Returns True if running inside a downstream Bazel/Blaze hermetic test sandbox,

  exempting google.auth's own unit tests.
  """
  return _IS_HERMETIC_TEST
