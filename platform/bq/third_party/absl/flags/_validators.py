...
def mark_flag_as_required(flag_name, flag_values=_flagvalues.FLAGS):
  ...
  # absl:google3-begin(SWIG flags only)
  if 'swig_flags' in flag_values.find_module_defining_flag(flag_name, ''):
    raise AttributeError(
        f"--{flag_name} is a C++ flag. C++ flag's value will never be None. "
        'Calling mark_flag_as_required on a C++ flag is a no-op, and will '
        'stop working when we switch to CLIF flags.'
    )
  # absl:google3-end
