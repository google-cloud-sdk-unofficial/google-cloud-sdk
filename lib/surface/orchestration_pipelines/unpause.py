...
class Unpause(calliope_base.Command):
  ...
  def Args(parser):
    ...
    parser.add_argument(
        "--pipeline",
        required=True,
        help="The ID of the pipeline to unpause.",
    )
    parser.add_argument(
        "--async",
        action="store_true",
        ...
        help=(
            ...
            " and unpause the pipeline asynchronously in the"
            " background."
        ),
    )
  ...
  def Run(self, args):
    ...
    # 1. Retrieve the pipeline to be activated.
    ...
    try:
      ...
      return {
          "result": "failed",
          "reason": (
              "An unexpected error occurred while unpausing the pipeline."
          ),
      }
    ...
    dags_util.ActivateDag(dag_ref)
    log.status.Print(f"Activating pipeline {args.pipeline}...")

    if args.async_:
      log.status.Print(
          "Pipeline files are updated and pipeline is being activated. Running"
          " in async mode, exiting..."
      )
      return {"result": "success"}
