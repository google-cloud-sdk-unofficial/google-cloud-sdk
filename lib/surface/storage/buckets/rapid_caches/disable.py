...
"""Implementation of disable command for Rapid Cache Ultra instances."""
...
class Disable(base.Command):
  """Disable Rapid Cache Ultra instances."""

  detailed_help = {
      'DESCRIPTION': (
          """
      Disable one or more Rapid Cache Ultra instances.
      """
      ),
      'EXAMPLES': (
          """
      The following command disables the Rapid Cache Ultra instance of bucket
      ``my-bucket'' having cache ID ``my-cache-id'':

        $ {command} my-bucket/my-cache-id

      The following command disables multiple Rapid Cache Ultra instances:

        $ {command} bucket-1/cache-1 bucket-2/cache-2
      """
      ),
  }
