...
"""Command to start a Cloud FTP server."""
...
class Start(base.Command):
  ...
  detailed_help = {
      ...
      'EXAMPLES': (
          """\
          To start server `my-server` in `us-west1`, run:
  ...
  def Args(parser):
    parser.add_argument(
        'SERVER_ID',
        help='The ID of the FTP server to start.',
    )
  ...
  def Run(self, args):
    ...
    op = client.StartServer(args.location, args.SERVER_ID)

    op_ref = operations_util.GetOperationRef(op.name)
    return client.WaitForOperation(
        op_ref,
        'Waiting for server [{}] to start'.format(args.SERVER_ID),
