...
DETAILED_HELP = {
    'DESCRIPTION':
        """\
      Creates a Custom Constraint from a JSON or YAML file. The custom
      constraint will be created if it does not exist, otherwise an error is thrown.
      """,
    'EXAMPLES':
        """\
...
class SetCustomConstraint(base.Command):
  ...
  def Run(self, args):
    """Creates custom constraint from a JSON or YAML file.

    This first converts the contents of the specified file into a custom
    constraint object. It then creates a custom constraint using
    ...
    return org_policy_api.CreateCustomConstraint(input_custom_constraint)
