...
"""Command to set the IAM policy for a BigLake Hive catalog database."""
...
class SetIamPolicy(base.Command):
  """Set the IAM policy for a BigLake Hive catalog database."""

  detailed_help = {
      'DESCRIPTION': 'Sets the IAM policy for a BigLake Hive catalog database.',
      'EXAMPLES': (
          """\
          To set the IAM policy for the catalog `my-catalog` and database `my-database` with the policy in `policy.json` run:

            $ {command} my-database policy.json --catalog=my-catalog
          """
      ),
  }

  # Iceberg and Hive share the same set_iam_policy method, but Iceberg has
