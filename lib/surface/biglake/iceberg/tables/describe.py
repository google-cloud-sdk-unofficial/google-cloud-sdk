...
"""The describe command for BigLake Iceberg REST catalogs tables."""
...
help_text = textwrap.dedent("""\
    To describe table `my-table` in parent catalog `my-catalog` and namespace `my-namespace`, run:
...
class Describe(base.DescribeCommand):
  ...
  @classmethod
  def Args(cls, parser):
    flags.AddTableResourceArg(parser, 'to describe', positional=True)
  ...
  def Run(self, args):
    ...
    request = messages.BiglakeIcebergV1RestcatalogV1ProjectsCatalogsNamespacesTablesGetRequest(
        name=table_name,
        snapshots=args.snapshots,
    )

    return client.iceberg_v1_restcatalog_v1_projects_catalogs_namespaces_tables.Get(
        request
    )
