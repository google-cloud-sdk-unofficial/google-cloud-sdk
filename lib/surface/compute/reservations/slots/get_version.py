...
"""Command for getting the version of a reservation slot."""
...
class GetVersion(base.Command):
  ...
  def Run(self, args):
    ...
    sbom_selections_enum = (
        messages.ReservationSlotsGetVersionRequest.SbomSelectionsValueListEntryValuesEnum
    )
    sbom_selections = []
    if args.current:
    ...
    body_message = messages.ReservationSlotsGetVersionRequest()
    if sbom_selections:
      body_message.sbomSelections = sbom_selections

    # Create the outer request instance with path/query parameters
    request = messages.ComputeReservationSlotsGetVersionRequest(
        ...
        reservationSlotsGetVersionRequest=body_message
    )

    errors = []
    responses = client.AsyncRequests(
        [(client.apitools_client.reservationSlots, 'GetVersion', request)],
    ...
    operation_poller = GetVersionPoller(
        holder.client.apitools_client.reservationSlots,
...
GetVersion.detailed_help = {
    'EXAMPLES': (
        """\
    To get the current version of a reservation slot in reservation
    exr-1 in ZONE with block name block-1, sub-block name sub-block-1
    and slot name slot-1, run:

      $ {command} exr-1 --zone=ZONE --block-name=block-1 \
          --sub-block-name=sub-block-1 --slot-name=slot-1 --current
    """
    ),
}
