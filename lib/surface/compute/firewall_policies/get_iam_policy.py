...
"""Command to get IAM policy for an organization firewall policy."""
...
class GetIamPolicy(base.Command):
  ...
  def Run(self, args):
    ...
    return org_firewall_policy.GetIamPolicy(fp_id=fp_id)
...
GetIamPolicy.detailed_help = {
    ...
    'DESCRIPTION':
        """\
        Get the IAM policy associated with a Compute Engine organization firewall policy.
        """,
    'EXAMPLES':
        """\
        To get the IAM policy for the organization firewall policy `my-policy`
        (under organization `123456789`), run:

          $ {command} my-policy --organization=123456789

        To get the IAM policy for the organization firewall policy with numeric ID `987654321`, run:
