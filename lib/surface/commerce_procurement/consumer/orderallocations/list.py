...
class List(base.ListCommand):
  """Returns the List of OrderAllocation objects resulting from the List API."""
  ...
  def Args(parser):
    ...
    resource_args.AddOrderParentResourceArg(
        parser, 'Parent Order to list order allocations.')
  ...
  def Run(self, args):
    ...
    Returns:
      List of order allocations and next token if applicable.
    """
    order_ref = args.CONCEPTS.order.Parse()
    return apis.OrderAllocations.List(order_ref.RelativeName(), args.page_size,
                                      args.page_token)
