...
"""Update GitLab Enterprise trigger command."""
...
class UpdateGitLabEnterprise(base.UpdateCommand):
  ...
  detailed_help = {
      'EXAMPLES':
          """\
            ...
            To update the 2nd-gen repository config of the trigger:
  ...
  def Args(parser):
    ...
    v1_config.add_argument(
        '--project-namespace', help='GitLab project namespace.')
    v1_config.add_argument(
...
"""),
  ...
  def ParseTriggerFromFlags(self, args, old_trigger, update_mask):
    ...
    # 1st-gen GitLab Enterprise config
    if args.project_namespace or args.gitlab_config_resource:
      ...
      update_mask.append('repository_event_config')
    elif args.repository:  # 2nd-gen (Repos API) config
      repo = messages.RepositoryEventConfig(repository=args.repository)
      trigger.repositoryEventConfig = repo
      update_mask.append('gitlab_enterprise_events_config')

    # Only updating event information.
    if args.branch_pattern:
      if self._isGleConfig(args, old_trigger):
        if trigger.gitlabEnterpriseEventsConfig is None:
          trigger.gitlabEnterpriseEventsConfig = messages.GitLabEventsConfig()
        trigger.gitlabEnterpriseEventsConfig.push = messages.PushFilter(
            branch=args.branch_pattern
        )
        update_mask.append('gitlab_enterprise_events_config.pull_request')
      else:
    ...
    if args.pull_request_pattern:
      ...
      if self._isGleConfig(args, old_trigger):
        if trigger.gitlabEnterpriseEventsConfig is None:
          trigger.gitlabEnterpriseEventsConfig = messages.GitLabEventsConfig()
        trigger.gitlabEnterpriseEventsConfig.pullRequest = (
            messages.PullRequestFilter(
                branch=pull_request_pattern,
            )
        )
        update_mask.append('gitlab_enterprise_events_config.push')
      else:
        if trigger.repositoryEventConfig is None:
          trigger.repositoryEventConfig = messages.RepositoryEventConfig()
    ...
    if args.comment_control:
      if self._isGleConfig(args, old_trigger):
        if trigger.gitlabEnterpriseEventsConfig is None:
          trigger.gitlabEnterpriseEventsConfig = messages.GitLabEventsConfig()
        if trigger.gitlabEnterpriseEventsConfig.pullRequest is None:
          trigger.gitlabEnterpriseEventsConfig.pullRequest = (
              messages.PullRequestFilter()
          )
        trigger.gitlabEnterpriseEventsConfig.pullRequest.commentControl = (
  ...
  def _isGleConfig(self, args, old_trigger):
    if args.gitlab_config_resource or args.project_namespace:
      return True
    if args.repository:
      return False
    if old_trigger.gitlabEnterpriseEventsConfig:
      return True
    return False
  ...
  def Run(self, args):
    ...
    Returns:
      The updated GitLab Enterprise trigger.
    """
