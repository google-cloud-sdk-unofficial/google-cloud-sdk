...
"""Command for omnichannel-gateway endpoint-configs create."""
...
class Create(base.CreateCommand):
  """Create an omnichannel endpoint configuration."""
  ...
  def Args(parser):
    ...
    parser.add_argument(
        ...
        nargs='?',
        help=(
            'ID of the endpoint configuration to create. If not specified, a'
    ...
    parser.add_argument(
        ...
        help='The endpoint configuration to create.',
    )
    base.ASYNC_FLAG.AddToParser(parser)
    parser.display_info.AddFormat('yaml')

  def Run(self, args):
    """Run endpoint-configs create command."""
    location_ref = args.CONCEPTS.location.Parse()
    project = location_ref.projectsId
    location = location_ref.locationsId
    endpoint_config_id = args.endpoint_config_id
    endpoint_config_body = args.endpoint_config

    url = agentic_utils.GetEndpointConfigsUrl(project, location)

    # In AIP-133 standard REST, POST creation parameters are passed as
    # query fields.
    params = {}
    if endpoint_config_id:
      params['endpointConfigId'] = endpoint_config_id

    client = api_client.OmnichannelGatewayClient()
    operation = client.Request(
        'POST',
        url,
        body=endpoint_config_body,
        params=params,
        error_msg='Failed to create endpoint config',
    )
    ...
    return agentic_utils.WaitForOperation(
        operation,
        client.session,
        'Waiting for omnichannel endpoint configuration to be created',
    )
