...
"""Command to cancel a Device Run session."""
...
class Cancel(base.Command):
  ...
  @staticmethod
  def Args(parser):
    resource_args.AddSessionResourceArg(parser, 'cancel')

  def Run(self, args):
    session_ref = args.CONCEPTS.session.Parse()
    client = device_run.SessionsClient(api_version='v1alpha')
    return client.Cancel(session_ref)


Cancel.detailed_help = {
    'DESCRIPTION': '{command} cancels a running Device Run session.',
    'EXAMPLES': (
        """\
To cancel a running Device Run session in location us-central1, run:
