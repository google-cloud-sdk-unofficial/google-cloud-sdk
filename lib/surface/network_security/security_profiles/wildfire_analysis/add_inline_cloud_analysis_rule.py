...
"""Add inline cloud analysis rule command to a WildFire Analysis profile."""
...
DETAILED_HELP = {
    'DESCRIPTION': """
          Add a WildFire inline cloud analysis rule to a WildFire Analysis Security Profile.
        """,
    'EXAMPLES': """
          To add an inline cloud analysis rule to a WildFire Analysis Security Profile named
      ...
      if file_type_choice not in FILE_TYPE_CHOICES:
        raise exceptions.InvalidArgumentException(
            ...
            'Invalid file type: {0}. Must be one of {1}'.format(
                file_type_choice, ', '.join(FILE_TYPE_CHOICES)
            ),
        )
      file_types_choices.append(file_type_choice)

    # If ANY_FILE is specified, no other file types can be specified.
    if 'ANY_FILE' in file_types_choices and len(file_types_choices) > 1:
      raise exceptions.InvalidArgumentException(
          '--file-types',
          'If ANY_FILE is specified, no other file types can be specified.',
      )

    response = client.AddInlineCloudAnalysisRule(
        security_profile.RelativeName(),
        file_types_choices=file_types_choices,
