...
"""Command that resumes an environment."""
...
DETAILED_HELP = {
    'EXAMPLES': (
        """\
          To resume the Cloud Composer environment named
          ``env-1'', run:
...
class Resume(base.Command):
  ...
  @staticmethod
  def Args(parser):
    resource_args.AddEnvironmentResourceArg(parser, 'to resume')
    base.ASYNC_FLAG.AddToParser(parser)

  def Run(self, args):
    env_resource = args.CONCEPTS.environment.Parse()
    operation = environments_api_util.Resume(
  ...
  def _SynchronousExecution(self, env_resource, operation):
    try:
      operations_api_util.WaitForOperation(
          operation,
          'Waiting for [{}] to be resumed with [{}]'.format(
      ...
      raise command_util.Error(
          'Error resuming environment [{}]: {}'.format(
