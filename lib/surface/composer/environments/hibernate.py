...
"""Command that hibernates an environment."""
...
DETAILED_HELP = {
    'EXAMPLES': (
        """\
          To hibernate the Cloud Composer environment named
          ``env-1'', run:
...
class Hibernate(base.Command):
  ...
  @staticmethod
  def Args(parser):
    resource_args.AddEnvironmentResourceArg(parser, 'to hibernate')
    base.ASYNC_FLAG.AddToParser(parser)

  def Run(self, args):
    env_resource = args.CONCEPTS.environment.Parse()
    operation = environments_api_util.Hibernate(
  ...
  def _SynchronousExecution(self, env_resource, operation):
    try:
      operations_api_util.WaitForOperation(
          operation,
          'Waiting for [{}] to be hibernated with [{}]'.format(
      ...
      raise command_util.Error(
          'Error hibernating environment [{}]: {}'.format(
