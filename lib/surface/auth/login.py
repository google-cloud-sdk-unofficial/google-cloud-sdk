...
class Login(base.Command):
  ...
  def Args(parser):
    ...
    parser.add_argument(
        ...
        'identity pool, generated by the Cloud Console or '
  ...
  def Run(self, args):
    ...
    login_config = vars(args).get('login_config', None)
    if args.cred_file:
      if login_config:
    ...
    login_config_file = _GetWorkforceLoginConfig(login_config)
    if login_config_file:
      ...
      if args.add_quota_project_to_adc:
        raise calliope_exceptions.ConflictingArgumentsException(
            ...
            'flow.')
      creds = DoWorkforceHeadfulLogin(login_config_file, **flow_params)
...
def _GetWorkforceLoginConfigFromEnvironment():
  return str(
      encoding.GetEncodedValue(os.environ, CLOUDSDK_AUTH_LOGIN_CONFIG_FILE, ''))


def _GetWorkforceLoginConfig(login_config_arg):
  ...
  Args:
    login_config_arg (Optional[str]): The argument supplied by `--cred-config=`
      if present.
  ...
  methods = [
      ('the --login-config commandline argument', login_config_arg),
      ('the CLOUDSDK_AUTH_LOGIN_CONFIG_FILE environment variable',
       _GetWorkforceLoginConfigFromEnvironment()),
      ('the auth/login_config_file project config setting',
       properties.VALUES.auth.login_config_file.Get()),
  ]
  ...
  active_method, active_config = methods[0]
  if len(methods) > 1:
    selected_methods = [method[0] for method in methods]
    message = textwrap.dedent("""You have specified login configurations using
    multiple methods ({}). Using "{}" from {}""".format(
        ', '.join(selected_methods), active_config, active_method))
    log.warning(message)
...
def DoWorkforceHeadfulLogin(login_config_file, **kwargs):
  ...
  return auth_util.DoInstalledAppBrowserFlowGoogleAuth(
