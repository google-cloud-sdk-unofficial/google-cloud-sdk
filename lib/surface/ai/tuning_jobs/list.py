...
"""Command to list tuning jobs in Vertex AI."""
...
_DETAILED_HELP = {
    "EXAMPLES": (
        """ \
        To list the tuning jobs of project ``example'' in region
        ``us-central1'', run:
...
class ListGA(base.ListCommand):
  ...
  def Args(cls, parser):
    ...
    flags.AddUriFlags(
        parser,
        tuning_jobs_util.TUNING_JOB_COLLECTION,
  ...
  def Run(self, args):
    ...
    with endpoint_util.AiplatformEndpointOverrides(
      ...
      return client.TuningJobsClient(version=self._version).List(
