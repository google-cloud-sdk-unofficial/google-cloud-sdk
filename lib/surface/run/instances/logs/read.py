...
"""Command to read logs for an instance."""
...
class Read(base.Command):
  ...
  detailed_help = {
      ...
      'EXAMPLES': (
          """\
          To read log entries from for a Cloud Run instance, run:
          ...
          To read log entries written in a specific time window, run:
            ...
            $ {command} my-instance --log-filter='timestamp<="2015-05-31T23:59:59Z" AND timestamp>="2015-05-31T00:00:00Z"'

          To read up to 10 log entries in your instance payloads that include the
          word `SearchText` and format the output in `JSON` format, run:

            $ {command} my-instance --log-filter="textPayload:SearchText" --limit=10 --format=json
  ...
  def Run(self, args):
    ...
    filters.append('resource.labels.instance_name = %s \n' % args.instance)
    filters.append(
