...
class Switchover(base.Command):
  ...
  def Run(self, args: argparse.Namespace) -> dict[str, Any]:
    ...
    console_io.PromptContinue(
        message=(
            f'Blue green deployment {args.deployment} in project {project} and'
            f' region {region} will be switched over.'
        ),
        cancel_on_no=True,
    )

    try:
      request = sql_messages.SqlBlueGreenDeploymentsSwitchoverRequest(
          name=deployment_name,
          switchoverBlueGreenDeploymentRequest=sql_messages.SwitchoverBlueGreenDeploymentRequest(),
      )
      result_operation = sql_client.blueGreenDeployments.Switchover(request)
      operation_ref = self.client.resource_parser.Create(
      ...
      operations.OperationsV1Beta4.WaitForOperation(
          sql_client, operation_ref, 'Switching over blue-green deployment'
      )

      return sql_client.blueGreenDeployments.Get(
          sql_messages.SqlBlueGreenDeploymentsGetRequest(name=deployment_name)
      )
