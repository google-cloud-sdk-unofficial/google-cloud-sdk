...
"""Command to describe an image streaming cache."""
...
class Describe(base.DescribeCommand):
  ...
  detailed_help = {
      ...
      "EXAMPLES": textwrap.dedent(
          """\
          # Check if the artifact is prewarmed using flags
  ...
  def Args(parser):
    ...
    resource_group.add_argument(
        "ARTIFACT",
        nargs="?",
        help="The full resource name of the version or tag to check.",
    )
    ...
    version_or_tag_group.add_argument(
        "--version",
        help="The version to check (e.g., 'sha256:abc123').",
    )
    version_or_tag_group.add_argument(
        "--tag",
        help="The tag to check (e.g., 'latest').",
    )
  ...
  def Run(self, args):
    ...
    return ar_requests.CheckPrewarmedArtifact(
        client,
        messages,
        artifact_ref,
