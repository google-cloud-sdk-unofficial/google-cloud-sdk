...
"""`gcloud service-directory registration-policies update` command."""
...
class UpdateBeta(base.UpdateCommand):
  """Updates a registration policy."""

  detailed_help = {
      'EXAMPLES':
          """\
          To update a Service Directory registration policy, run:
  ...
  @staticmethod
  def Args(parser):
    resource_args.AddLocationResourceArg(parser, 'to update', positional=False)
  ...
  def Run(self, args):
    ...
    result = client.Update(location_ref, policy)
    log.UpdatedResource(policy['metadata']['name'], _RESOURCE_TYPE)
