...
class RemoveIamPolicyBindingCommand(base.Command):
  ...
  detailed_help = {
      'EXAMPLES':
          ...
          To remove an IAM policy binding for the role of
          'roles/iap.tunnelResourceAccessor' for the user 'test-user@gmail.com'
          to the Cloud Run service 'test-service' located in the region 'us-west1', run:

            $ {command} --member='user:test-user@gmail.com'
               --role='roles/iap.tunnelResourceAccessor'
               --resource-type=cloud-run --service=test-service --region=us-west1

          To remove all IAM policy bindings with a condition of expression='request.time < timestamp("2025-01-01T00:00:00Z")',
          title='expires_end_of_2024', and description='Expires at midnight on 2024-12-31'
          for the role of 'roles/iap.tunnelResourceAccessor' for the user 'test-user@gmail.com'
          to the Cloud Run service 'test-service' located in the region 'us-west1', run:
  ...
  def Run(self, args):
    ...
    iap_tcp_ref = iap_util.ParseIapTcpIamResource(
        self.ReleaseTrack(),
        args)
    return iap_tcp_ref.RemoveIamPolicyBinding(args.member, args.role, condition,
                                              args.all)
