...
def _NormalizeArgName(name):
  """Normalizes an argument name for consistent lookup in resource maps.

  Args:
    name: str, The argument name or flag string.

  Returns:
    str, The normalized argument name (lowercase, hyphens, no leading dashes).
  """
  if not name:
    return ''
...
# We should go from the narrowest / most specific base classes to the most
# broad / general base classes.
# Otherwise, the "isinstance" check below might return True for a more
# general base class when it should have returned True for a more specific
# base class.
_KNOWN_BASE_CLASSES = (
    (base.CreateCommand, 'CreateCommand'),
    (base.DeleteCommand, 'DeleteCommand'),
    (base.DescribeCommand, 'DescribeCommand'),
    (base.ListCommand, 'ListCommand'),
    (base.UpdateCommand, 'UpdateCommand'),
    (base.RestoreCommand, 'RestoreCommand'),
    (base.ImportCommand, 'ImportCommand'),
    (base.ExportCommand, 'ExportCommand'),
    (base.DeclarativeCommand, 'DeclarativeCommand'),
    (base.BinaryBackedCommand, 'BinaryBackedCommand'),
    (base.TopicCommand, 'TopicCommand'),
    (base.SilentCommand, 'SilentCommand'),
    (base.CacheCommand, 'CacheCommand'),
    (base.Group, 'Group'),
    (base.Command, 'Command'),
)


def _GetCommandBaseType(command):
  """Returns the base command type name for a calliope command or group.

  Default value is GENERIC if no match was found.

  Args:
    command: backend.CommandCommon, A Calliope command or group.

  Returns:
    str, The base command type name, or 'GENERIC'.
  """
  common_type = getattr(command, '_common_type', None)
  default = 'GENERIC'
  if not common_type or not isinstance(common_type, type):
    return default

  for base_cls, name in _KNOWN_BASE_CLASSES:
    try:
      if issubclass(common_type, base_cls):
        return name
    except TypeError:
      pass
...
class Command(object):
  ...
  Attributes:
    ...
    default_universe_compatible: bool, True if the command is compatible in the
      default universe.
    command_type: str, The base command type name (e.g. ListCommand, Group).
    name: str, The normalized name ('_' => '-').
  ...
  def __init__(self, command, parent):
    ...
    self.command_type = _GetCommandBaseType(command)
    ...
    # Collect all available argument names for this command to validate
    # ArgFallthrough references during resource attribute extraction.
    available_command_args = set()
    for arg in getattr(args, 'flag_args', []):
      for name in getattr(arg, 'option_strings', []):
        available_command_args.add(_NormalizeArgName(name))
    ...
    for arg in getattr(args, 'positional_args', []):
      ...
      if dest:
        available_command_args.add(_NormalizeArgName(dest))

    argument_resource_map = {}
    if concept_handler:
      from googlecloudsdk.calliope.concepts import deps

      # Helper to check if an argument is available on this command.
      def _IsArgAvailable(arg_n):
        return (
            not available_command_args
            or _NormalizeArgName(arg_n) in available_command_args
        )

      # Extract fallthrough sources (gcloud properties, flag fallthroughs,
      # static values) for a resource attribute.
      def _ExtractFallthroughs(fallthroughs_map):
        props, flags, values = [], [], []
        for _, attr_fts in six.iteritems(fallthroughs_map):
          for f in attr_fts:
            if isinstance(f, deps.PropertyFallthrough):
              ...
              if _IsArgAvailable(f.arg_name) and f.arg_name not in flags:
                flags.append(f.arg_name)
            elif isinstance(f, deps.ValueFallthrough):
              ...
              for sub_f in getattr(f, '_fallthroughs', []):
                if isinstance(sub_f, deps.ArgFallthrough):
                  if (
                      _IsArgAvailable(sub_f.arg_name)
                      and sub_f.arg_name not in flags
                  ):
                    flags.append(sub_f.arg_name)
                elif isinstance(sub_f, deps.PropertyFallthrough):
                  v = str(sub_f.property)
                  if v not in props:
                    props.append(v)
                elif isinstance(sub_f, deps.ValueFallthrough):
                  if getattr(sub_f, 'value', None) is not None:
                    v = str(sub_f.value)
                    if v not in values:
                      values.append(v)
        return props, flags, values

      # Register resource metadata for an argument name under a normalized key.
      def _RegisterArgMetadata(arg_name, res_metadata):
        key = _NormalizeArgName(arg_name)
        if key:
          argument_resource_map[key] = res_metadata
      ...
      for concept_details in concept_handler._all_concepts:
        ...
        if resource_spec and getattr(resource_spec, 'collection', None):
          ...
          if hasattr(info, 'BuildFullFallthroughsMap'):
            fallthroughs_map = info.BuildFullFallthroughsMap()
          elif hasattr(info, 'fallthroughs_map'):
            fallthroughs_map = info.fallthroughs_map
          else:
            fallthroughs_map = {}

          # Extract metadata per resource attribute (e.g. zone, project,
          # instance).
          attributes_dict = {}
          for attribute in getattr(resource_spec, 'attributes', []):
            # Resolve parameter name in API schema (e.g. 'zonesId' for 'zone').
            attr_name = getattr(attribute, 'name', '')
            try:
              p_name = resource_spec.ParamName(attr_name)
            except (ValueError, AttributeError):
              p_name = attr_name

            # Find bound CLI flag name (e.g. '--zone').
            arg_name = info.attribute_to_args_map.get(attr_name, '')
            if not arg_name and hasattr(info, 'attribute_to_args_map'):
              norm_attr = _NormalizeArgName(attr_name)
              for k, v in six.iteritems(info.attribute_to_args_map):
                if _NormalizeArgName(k) == norm_attr:
                  arg_name = v
                  break

            # Resolve help text (evaluate callable if necessary).
            help_text = getattr(attribute, 'help_text', '')
            ...
            # Extract fallthrough sources for this attribute.
            attr_fts = (
                fallthroughs_map.get(attr_name)
                or fallthroughs_map.get(p_name)
                or fallthroughs_map.get(attribute)
                or []
            )
            attr_map = {attr_name: attr_fts}
            attr_props, attr_flags, attr_values = _ExtractFallthroughs(attr_map)
            ...
            if arg_name:
              attr_info['flag'] = arg_name
            if help_text:
              attr_info['help_text'] = help_text
            if attr_props:
              attr_info['fallthroughs'] = attr_props
            if attr_flags:
              attr_info['flag_fallthroughs'] = attr_flags
            if attr_values:
              attr_info['value_fallthroughs'] = attr_values

            attributes_dict[p_name] = attr_info

          # Register resource metadata for each bound argument name and anchor
          # in argument_resource_map for O(1) normalized lookup when
          # processing flags, positionals, and constraint groups.
          for attr_name, arg_name in six.iteritems(info.attribute_to_args_map):
            ...
            if attributes_dict:
              res_metadata['attributes'] = attributes_dict

            _RegisterArgMetadata(arg_name, res_metadata)
            if anchor:
              _RegisterArgMetadata(anchor, res_metadata)
    ...
    for arg in args.flag_args:
      for name in arg.option_strings:
        if name.startswith('--'):
          ...
          name = name.replace('_', '-')
          res_meta = argument_resource_map.get(
              _NormalizeArgName(name)
          ) or argument_resource_map.get(_NormalizeArgName(arg.dest))
          flag = Flag(arg, name, resource_metadata=res_meta)
          flag.alternative_names = [
              alt for alt in arg.option_strings if alt != name
          ]
