...
def AddAcceleratorNetworkProfileNameArg(parser, help_text):
  """Adds a positional Accelerator Network Profile name argument to the parser."""
  parser.add_argument('name', metavar='NAME', help=help_text)
...
def AddAcceleratorNetworkProfileCreateFlags(parser, hidden=True):
  """Adds flags for Accelerator Network Profile creation to the parser."""
  parser.add_argument(
      '--cluster',
      required=True,
  ...
  parser.add_argument(
      '--description',
      help='A human-readable description of the profile.',
      hidden=hidden,
  )
  parser.add_argument(
      '--max-node-count',
      type=int,
      help="""\
      The maximum number of nodes this profile can support, influencing
      subnet sizes. If omitted, it defaults to supporting up to 65,536 nodes.
      """,
      hidden=hidden,
  )
  parser.add_argument(
      '--auto-config-machine-type',
      help=(
          'Required when using any auto-configuration options. Machine type to'
          ' base the configuration on.'
      ),
      hidden=hidden,
  )
  parser.add_argument(
      '--auto-config-mrdma-network-profile',
      help="""\
      The URL or ID of the Compute Engine Network Profile to associate with the accelerator NICs with MRDMA as type.
      If specified, this profile applies to all MRDMA accelerator NICs generated by the auto-configuration.
      """,
      hidden=hidden,
  )
  parser.add_argument(
      '--auto-create-local-mrdma-vpc',
      action='store_true',
      default=None,
      help="""\
      If specified, the AcceleratorNetworkProfile automatically creates
      non-shareable VPCs for MRDMA.
      """,
      hidden=hidden,
  )
  parser.add_argument(
  ...
  parser.add_argument(
      '--auto-config-host-nic-in-same-vpc',
      action='store_true',
      default=None,
      help="""\
      If specified, all host Network Interface Cards (Diorite NICs) will be placed within
      the same VPC with default NIC. This configuration is specific to certain machine types.
      """,
      hidden=hidden,
  )
  parser.add_argument(
      ...
      metavar='KEY=VALUE',
      help="""\
      A list of comma-separated key-value pairs to apply to this resource.
      """,
      hidden=hidden,
  )
  parser.add_argument(
      ...
      metavar='KEY=VALUE',
      help="""\
      A list of comma-separated key-value pairs to bind as Cloud Tags to the
      resource.
      """,
      hidden=hidden,
  )
  parser.add_argument(
      ...
      metavar='KEY=VALUE',
      help="""\
      A list of comma-separated key-value pairs to store small amounts of
      arbitrary data.
      """,
      hidden=hidden,
  )
...
def AddTimeoutFlag(parser, default=1800):
  """Adds the --timeout flag to parser."""
  parser.add_argument(
      '--timeout',
      type=int,
      default=default,
