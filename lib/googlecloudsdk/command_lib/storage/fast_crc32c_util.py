...
class _GcloudCrc32c(object):
  """Base class for CRC32C implementations using gcloud-crc32c."""

  def __init__(self, crc=0):
    """Initializes the CRC32C object with an initial checksum value.
...
class DeferredCrc32c(_GcloudCrc32c):
...
class OnTheFlyCrc32c(_GcloudCrc32c):
  """Hashlib-like helper for on-the-fly calculations using gcloud-crc32c.

  NOTE: This class does not support sum_file. This class is intended to be used
  only in case of streaming uploads/downloads, scenarios where the entire data
  is not available in disk/memory. Frequent and small on-the-fly crc32c
  calculations can slow down the command's overall speed.
  """

  def copy(self) -> 'OnTheFlyCrc32c':
    return OnTheFlyCrc32c(crc=self._crc)

  def update(self, data: bytes) -> None:
    """Updates the CRC32C checksum with the given data."""
    crc32c_operation = GcloudCrc32cOperation()
    result = crc32c_operation(file_path='-', stdin=data)
    if result.failed:
      log.debug(
          'Failed to calculate CRC32C for data chunk. CRC could not be updated.'
          ' %s',
          result.stderr,
      )
      raise errors.Error(
          'Failed to calculate CRC32C for data chunk. %s' % result.stderr
      )
    self._crc = crc32c.concat_checksums(
        self._crc, int(result.stdout), len(data)
    )

  def sum_file(self, file_path: str, offset: int, length: int) -> None:
    raise NotImplementedError('OnTheFlyCrc32c does not support sum_file.')
...
def get_crc32c(initial_data=b'', *, is_streaming=False):
  ...
  Args:
    initial_data (bytes): The CRC32C object will be initialized with the
      checksum of the data, incase of google-crc32c or crcmod.
    is_streaming (bool): If True, returns a CRC32C object that supports
      on-the-fly updates.

  Returns:
    It returns instance of DeferredCrc32c/OnTheFlyCrc32c if gcloud-crc32c is
    used. Otherwise it returns a google_crc32c.Checksum instance if
    google-crc32c(https://github.com/googleapis/python-crc32c) is available and
    a predefined.Crc instance from crcmod library if not.
  """
  use_gcloud_crc32c_binary = should_use_gcloud_crc32c(install_if_missing=True)
  if use_gcloud_crc32c_binary:
    return OnTheFlyCrc32c() if is_streaming else DeferredCrc32c()
  return crc32c.get_crc32c(initial_data)
