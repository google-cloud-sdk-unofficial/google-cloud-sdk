...
def AddComputeInstanceBackupPlanProperties(parser, release_track=None):
  ...
  # LINT.IfChange(selective_disk_backup_help)
  help_text = textwrap.dedent("""\
      ...
      *guest-flush*::: Indicates whether to perform a guest flush operation
      ...
      """)
  # NOTE: If you unhide 'boot-disk-only' or 'disk-exclusion-labels' in the spec below,
  # you MUST add their descriptions back to the help_text string above.
  # LINT.ThenChange(:selective_disk_backup_hidden)
  spec = {
      'guest-flush': arg_parsers.ArgBoolean(),
  }
  if release_track == base.ReleaseTrack.ALPHA:
    # LINT.IfChange(selective_disk_backup_hidden)
    spec['boot-disk-only'] = arg_parsers.ArgObject(
        value_type=bool,
        hidden=True
    )
    spec['disk-exclusion-labels'] = arg_parsers.ArgObject(
        value_type=str,
        hidden=True
    )
    # LINT.ThenChange(:selective_disk_backup_help)
