...
def AddComputeInstanceBackupPlanProperties(parser, release_track=None):
  """Adds the --compute-instance-properties flag to the given parser."""
  ...
  help_text = textwrap.dedent("""\
      ...
      *boot-disk-only*::: Indicates whether to backup only the boot disk of the
      compute instance. When set to true, only the boot disk is backed up.

      *disk-exclusion-labels*::: Key-value pairs of labels used to identify
      disks to be excluded from backup for the compute instance.

      Examples:
        --compute-instance-properties=guest-flush=true,boot-disk-only=true
        --compute-instance-properties=disk-exclusion-labels=backup=exclude
      """)
  spec = {
      'guest-flush': arg_parsers.ArgBoolean(),
      'boot-disk-only': arg_parsers.ArgObject(
          value_type=bool,
      ),
      'disk-exclusion-labels': arg_parsers.ArgObject(
          value_type=str,
      ),
  }
