...
"""Download utils for Agent Registry commands."""
...
def Download(
    dest_path: str,
    revision_res_name: str,
    file_name: str,
    allow_overwrite: bool,
    chunk_size: int,
    parallelism: int = 1,
) -> None:
  """Downloads a revision payload to a local path."""
  ...
  request_class = getattr(
      messages, 'AgentregistryProjectsLocationsSkillsRevisionsGetRequest'
  )
  file_req = request_class(name=revision_res_name)
  file_res = client.projects_locations_skills_revisions.Get(file_req)

  if not file_res.sizeBytes:
    # Backend did not provide a sizeBytes attribute.
    # Fallback to standard apitools streaming download (no parallel chunking).
    download = transfer.Download.FromFile(dest_path, overwrite=allow_overwrite)
    download.bytes_http = transports.GetApitoolsTransport(
        response_encoding=None
    )

    client.projects_locations_skills_revisions.Get(file_req, download=download)
    return
