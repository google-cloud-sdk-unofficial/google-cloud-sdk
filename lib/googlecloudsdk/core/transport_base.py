...
DryRunError = exceptions.DryRunError
...
def _NoopResponseHandler(*unused_args, **unused_kwargs):
  """No-op response handler used in place of lambdas."""
  return None
...
def LogRequest(redact_token=True, redact_request_body_reason=None):
  ...
  def _LogRequest(request):
    ...
    for h, v in sorted(six.iteritems(headers)):
      if redact_token and h.lower() in (
          b'authorization',
...
def GetAndCacheArchitecture(user_platform):
  ...
  if (
      user_platform.operating_system == platforms.OperatingSystem.MACOSX
      and user_platform.architecture == platforms.Architecture.x86_64
      and platforms.Platform.IsActuallyM1ArmArchitecture()
  ):
    arch = '{}_{}'.format(
...
def IsTokenUri(uri):
  ...
  metadata_regexp = (
      ...
      r'computeMetadata/.*?/instance/service-accounts/.*?/token'
  )

  impersonate_service_account = (
      r'iamcredentials\.googleapis\.com/v.*?/projects/'
...
def GetValidMCPMetricsString():
  ...
  if 'GCLOUD_MCP_METRICS' in os.environ:
