...
class EdgecontainerProjectsLocationsMachinesListRequest(_messages.Message):
  ...
  Fields:
    filter: Only resources matching this filter will be listed.
    orderBy: Specifies the order in which machines will be listed. In
      accordance with AIP-132, the value should be a comma-separated list of
      fields. A " desc" may be appended to a field to specify descending
      order. Supported fields: * `name` (default) : Canonical resource name of
      the machine. Example: "name desc".
    pageSize: The maximum number of resources to list.
    pageToken: A page token received from previous list request.
...
class OperationMetadata(_messages.Message):
  ...
  Fields:
    ...
    publicError: A public-facing error representation, intended for client
      consumption. This field is populated from the operation's error, and is
      especially useful for providing richer error details to clients when the
      canonical error code is `INTERNAL` and the primary error message may be
      sanitized.
  ...
  publicError = _messages.MessageField('Status', 4)
  requestedCancellation = _messages.BooleanField(5)
  statusMessage = _messages.StringField(6)
  statusReason = _messages.EnumField('StatusReasonValueValuesEnum', 7)
  target = _messages.StringField(8)
  verb = _messages.StringField(9)
  warnings = _messages.StringField(10, repeated=True)
