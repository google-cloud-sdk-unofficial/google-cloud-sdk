...
class FirestoreProjectsDatabasesDocumentsListCollectionIdsRequest(_messages.Message):
  ...
  Fields:
    ...
    parent: Required. The parent document. In the format:
      ...
      `projects/{project_id}/databases/{database_id}/documents` to list top-
      level collections.
  """
...
class GoogleFirestoreAdminV1Backup(_messages.Message):
  ...
  Fields:
    ...
    name: Output only. The unique resource name of the Backup. Format is
      `projects/{project}/locations/{location}/backups/{backup}`. The location
      in the name will be the Standard Managed Multi-Region (SMMR) location
      (e.g. `us`) if the backup was created with an SMMR location, or the
      Google Managed Multi-Region (GMMR) location (e.g. `nam5`) if the backup
...
class GoogleFirestoreAdminV1Database(_messages.Message):
  ...
  Enums:
    ...
    TypeValueValuesEnum: Required. The type of the database. See
  ...
  Fields:
    ...
    keyPrefix: Output only. The key_prefix for this database. This key_prefix
      ...
      instead of v~foo).
    locationId: Required. The location of the database. Available locations
  ...
  class TypeValueValuesEnum(_messages.Enum):
    r"""Required. The type of the database. See
    https://cloud.google.com/datastore/docs/firestore-or-datastore for
    information about how to choose.
...
class StructuredQuery(_messages.Message):
  ...
  Fields:
    ...
    orderBy: The order to apply to the query results. Callers can provide a
      full ordering, a partial ordering, or no ordering at all. While
      Firestore will always respect the provided order, the behavior for
      queries without a full ordering is different per database edition: In
      ...
      most efficient ordering based on the indexes available at the time of
      query execution. This will result in a different ordering for queries
      that are otherwise identical. To ensure a stable ordering, always
      include a unique field in the `order_by` clause, such as `__name__`.
...
class Value(_messages.Message):
  ...
  Fields:
    ...
    variableReferenceValue: Pointer to a variable defined elsewhere in a
      pipeline. Unlike `field_reference_value` which references a field within
      a document, this refers to a variable, defined in a separate namespace
      than the fields of a document.
  """
  ...
  variableReferenceValue = _messages.StringField(15)
