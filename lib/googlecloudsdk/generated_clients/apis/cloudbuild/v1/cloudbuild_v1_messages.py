...
class BranchFilter(_messages.Message):
  r"""Specifies a branch filter using regex.

  Fields:
    globs: Optional. Glob patterns to match branches.
    nameRegexes: Optional. RE2 regex patterns to match branches.
  """

  globs = _messages.StringField(1, repeated=True)
  nameRegexes = _messages.StringField(2, repeated=True)
...
class Build(_messages.Message):
  ...
  Messages:
    ParamsValue: Optional. Parameters for the Build resource. Note: In the
      MVP, this field is only supported for builds executing within a Cloud
      Build Pipeline. Specifying `params` on a standalone build will result in
  ...
  Fields:
    ...
    params: Optional. Parameters for the Build resource. Note: In the MVP,
      this field is only supported for builds executing within a Cloud Build
      Pipeline. Specifying `params` on a standalone build will result in an
      INVALID_ARGUMENT error.
    pipelineRunRef: Output only. Reference to the PipelineRun and Stage that
      created this build.
  ...
  class ParamsValue(_messages.Message):
    r"""Optional. Parameters for the Build resource. Note: In the MVP, this
    field is only supported for builds executing within a Cloud Build
    Pipeline. Specifying `params` on a standalone build will result in an
    INVALID_ARGUMENT error.
  ...
  pipelineRunRef = _messages.MessageField('PipelineRunRef', 17)
  projectId = _messages.StringField(18)
  queueTtl = _messages.StringField(19)
  remoteConfig = _messages.StringField(20)
  results = _messages.MessageField('Results', 21)
  secrets = _messages.MessageField('Secret', 22, repeated=True)
  serviceAccount = _messages.StringField(23)
  source = _messages.MessageField('Source', 24)
  sourceProvenance = _messages.MessageField('SourceProvenance', 25)
  startTime = _messages.StringField(26)
  status = _messages.EnumField('StatusValueValuesEnum', 27)
  statusDetail = _messages.StringField(28)
  steps = _messages.MessageField('BuildStep', 29, repeated=True)
  substitutions = _messages.MessageField('SubstitutionsValue', 30)
  tags = _messages.StringField(31, repeated=True)
  timeout = _messages.StringField(32)
  timing = _messages.MessageField('TimingValue', 33)
  warnings = _messages.MessageField('Warning', 34, repeated=True)
...
class BuildDefaults(_messages.Message):
  r"""Default settings applied to all builds in the pipeline.

  Enums:
    LoggingModeValueValuesEnum: Optional. The default logging mode applied to
      builds in the pipeline.

  Fields:
    loggingMode: Optional. The default logging mode applied to builds in the
      pipeline.
    pubsubTopic: Optional. The default Pub/Sub topic for individual build
      updates.
    queueTtl: Optional. The default queue TTL applied to builds in the
      pipeline.
    serviceAccount: Optional. The default Service Account used by Stages
      (Builds) in this Pipeline. If a Stage does not specify its own
      service_account, this will be used.
    timeout: Optional. The default timeout applied to builds in the pipeline.
    workerPool: Optional. The default worker pool used by Stages in this
      Pipeline.
  """

  class LoggingModeValueValuesEnum(_messages.Enum):
    r"""Optional. The default logging mode applied to builds in the pipeline.
  ...
  loggingMode = _messages.EnumField('LoggingModeValueValuesEnum', 1)
  pubsubTopic = _messages.StringField(2)
  queueTtl = _messages.StringField(3)
...
class BuildSecurityPolicy(_messages.Message):
  ...
  Fields:
    ...
    networkAllowedUrlPrefix: Optional. URL prefixes that are allowed to be
      accessed during the build. If using redirects, the final URL must be in
      this list. If this field is empty, all URL prefixes are allowed.
  ...
  networkAllowedUrlPrefix = _messages.StringField(4, repeated=True)
  workflowType = _messages.EnumField('WorkflowTypeValueValuesEnum', 5)


class BuildStage(_messages.Message):
  r"""The build information for a stage of the pipeline.

  Enums:
    StateValueValuesEnum: Output only. Current execution state of the stage.

  Messages:
    EvaluatedParamsValue: Output only. Resolved stage-level parameters.

  Fields:
    buildInfo: Output only. The audit trail of the remote fetch.
    buildSnapshot: Output only. If the Stage referenced a remote repository,
      this holds the unmodified build fetched from that repository.
    endTime: Output only. Execution finish timestamp.
    evaluatedBuild: Output only. The fully flattened, CEL-evaluated
      configuration that was dispatched with the build.
    evaluatedParams: Output only. Resolved stage-level parameters.
    results: Output only. Dynamic outputs produced by this stage.
    startTime: Output only. Execution start timestamp.
    state: Output only. Current execution state of the stage.
  """

  class StateValueValuesEnum(_messages.Enum):
    r"""Output only. Current execution state of the stage.

    Values:
      EXECUTION_STATE_UNSPECIFIED: Default value. Used when the execution
        state is unknown or not set.
      PENDING: Waiting for resources or upstream dependencies.
      RUNNING: Execution is actively occurring.
      SUCCEEDED: Execution completed successfully.
      FAILED: Execution encountered an error.
      CANCELLED: Cancelled by user.
      TIMED_OUT: Exceeded wall-clock timeout.
      SKIPPED: (Stages Only) Skipped because an upstream dependency failed.
    """
  ...
  @encoding.MapUnrecognizedFields('additionalProperties')
  class EvaluatedParamsValue(_messages.Message):
    r"""Output only. Resolved stage-level parameters.

    Messages:
      AdditionalProperty: An additional property for a EvaluatedParamsValue
        object.

    Fields:
      additionalProperties: Additional properties of type EvaluatedParamsValue
    """

    class AdditionalProperty(_messages.Message):
      r"""An additional property for a EvaluatedParamsValue object.
  ...
  buildInfo = _messages.MessageField('SnapshotInfo', 1)
  buildSnapshot = _messages.MessageField('Build', 2)
  endTime = _messages.StringField(3)
  evaluatedBuild = _messages.MessageField('Build', 4)
  evaluatedParams = _messages.MessageField('EvaluatedParamsValue', 5)
...
class CancelPipelineRunRequest(_messages.Message):
  r"""Message for cancelling a PipelineRun."""


class CelFilter(_messages.Message):
  r"""Represents a CEL expression filter.

  Fields:
    expression: Optional. The CEL expression, must evaluate to true or false.
    notification: Optional. Optional message to return to the SCM if the
...
class CloudbuildProjectsLocationsPipelineRunsCancelRequest(_messages.Message):
  r"""A CloudbuildProjectsLocationsPipelineRunsCancelRequest object.

  Fields:
    cancelPipelineRunRequest: A CancelPipelineRunRequest resource to be passed
      as the request body.
    name: Required. Format:
      `projects/{project}/locations/{location}/pipelineRuns/{pipeline_run}`
  """

  cancelPipelineRunRequest = _messages.MessageField('CancelPipelineRunRequest', 1)
  name = _messages.StringField(2, required=True)
...
class CloudbuildProjectsLocationsPipelineRunsCreateRequest(_messages.Message):
  ...
  Fields:
    ...
    pipelineRunId: Optional. The ID to use for the PipelineRun.
  """
...
class CloudbuildProjectsLocationsPipelineRunsListRequest(_messages.Message):
  ...
  Fields:
    filter: Optional. Filter for the results.
    orderBy: Optional. The order to sort results by.
    pageSize: Optional. Number of results to return in the list.
...
class CloudbuildProjectsLocationsPipelinesCreateRequest(_messages.Message):
  r"""A CloudbuildProjectsLocationsPipelinesCreateRequest object.

  Fields:
    parent: Required. Format: `projects/{project}/locations/{location}`
    pipeline: A Pipeline resource to be passed as the request body.
    pipelineId: Required. The ID to use for the Pipeline, which will become
      the final component of the pipeline's resource name.
...
class CloudbuildProjectsLocationsPipelinesDeleteRequest(_messages.Message):
  r"""A CloudbuildProjectsLocationsPipelinesDeleteRequest object.

  Fields:
    etag: The etag of the Pipeline. If this is provided, it must match the
      server's etag.
    name: Required. Format:
...
class CloudbuildProjectsLocationsPipelinesGetRequest(_messages.Message):
  r"""A CloudbuildProjectsLocationsPipelinesGetRequest object.
...
class CloudbuildProjectsLocationsPipelinesListRequest(_messages.Message):
  r"""A CloudbuildProjectsLocationsPipelinesListRequest object.
...
class CloudbuildProjectsLocationsPipelinesPatchRequest(_messages.Message):
  r"""A CloudbuildProjectsLocationsPipelinesPatchRequest object.

  Fields:
    allowMissing: If set to true, and the pipeline is not found, a new
      pipeline will be created. In this situation, `update_mask` is ignored.
    name: Identifier. The name of the Pipeline. Format:
      `projects/{project}/locations/{location}/pipelines/{pipeline}`
    pipeline: A Pipeline resource to be passed as the request body.
...
class CloudbuildProjectsLocationsPipelinesTriggersCreateRequest(_messages.Message):
  r"""A CloudbuildProjectsLocationsPipelinesTriggersCreateRequest object.

  Fields:
    parent: Required. Format:
      `projects/{project}/locations/{location}/pipelines/{pipeline}`
    pipelineTrigger: A PipelineTrigger resource to be passed as the request
      body.
    pipelineTriggerId: Required. The ID to use for the PipelineTrigger, which
      will become the final component of the pipeline trigger's resource name.
    validateOnly: Optional. If set, validates the request and previews the
      result without executing creation.
  """

  parent = _messages.StringField(1, required=True)
  pipelineTrigger = _messages.MessageField('PipelineTrigger', 2)
  pipelineTriggerId = _messages.StringField(3)
  validateOnly = _messages.BooleanField(4)


class CloudbuildProjectsLocationsPipelinesTriggersDeleteRequest(_messages.Message):
  r"""A CloudbuildProjectsLocationsPipelinesTriggersDeleteRequest object.

  Fields:
    etag: Optional. The etag of the PipelineTrigger.
    name: Required. Format: `projects/{project}/locations/{location}/pipelines
      /{pipeline}/triggers/{trigger}`
    validateOnly: Optional. If set, validates the request and previews the
      result without executing deletion.
  """
...
class CloudbuildProjectsLocationsPipelinesTriggersGetRequest(_messages.Message):
  r"""A CloudbuildProjectsLocationsPipelinesTriggersGetRequest object.
...
class CloudbuildProjectsLocationsPipelinesTriggersListRequest(_messages.Message):
  r"""A CloudbuildProjectsLocationsPipelinesTriggersListRequest object.
...
class CloudbuildProjectsLocationsPipelinesTriggersPatchRequest(_messages.Message):
  r"""A CloudbuildProjectsLocationsPipelinesTriggersPatchRequest object.

  Fields:
    allowMissing: Optional. If set to true, and the PipelineTrigger is not
      found, a new PipelineTrigger will be created. In this situation,
      `update_mask` is ignored.
    name: Identifier. The resource name of the trigger. Format: `projects/{pro
      ject}/locations/{location}/pipelines/{pipeline}/triggers/{trigger}`
    pipelineTrigger: A PipelineTrigger resource to be passed as the request
      body.
    updateMask: Optional. The list of fields to be updated.
    validateOnly: Optional. If set, validates the request and previews the
      result without executing the update.
  """

  allowMissing = _messages.BooleanField(1)
  name = _messages.StringField(2, required=True)
  pipelineTrigger = _messages.MessageField('PipelineTrigger', 3)
  updateMask = _messages.StringField(4)
  validateOnly = _messages.BooleanField(5)


class CloudbuildProjectsLocationsPipelinesTriggersReceiveRequest(_messages.Message):
  r"""A CloudbuildProjectsLocationsPipelinesTriggersReceiveRequest object.

  Fields:
    httpBody: A HttpBody resource to be passed as the request body.
    name: Required. The resource name of the PipelineTrigger. Format: `project
      s/{project}/locations/{location}/pipelines/{pipeline}/triggers/{trigger}
      `
    secretToken: Required. The secret token used for authorization.
  """

  httpBody = _messages.MessageField('HttpBody', 1)
  name = _messages.StringField(2, required=True)
  secretToken = _messages.StringField(3)
...
class GitRefFilter(_messages.Message):
  r"""Specifies a Git ref filter using globs or regex.

  Fields:
    globs: Optional. Glob patterns to match branches/tags (ORed).
    nameRegexes: Optional. RE2 regex patterns to match branches/tags (ORed).
  """
...
class ListPipelineRunsResponse(_messages.Message):
  ...
  Fields:
    nextPageToken: A token identifying a page of results the server should
      return.
    pipelineRuns: The list of PipelineRuns.
  """
...
class ListPipelineTriggersResponse(_messages.Message):
  r"""Message for response to listing PipelineTriggers.

  Fields:
    nextPageToken: A token identifying a page of results the server should
      return.
    pipelineTriggers: The list of PipelineTriggers.
  """

  nextPageToken = _messages.StringField(1)
  pipelineTriggers = _messages.MessageField('PipelineTrigger', 2, repeated=True)


class ListPipelinesResponse(_messages.Message):
  r"""Message for response to listing Pipelines.
...
class MergeQueueFilter(_messages.Message):
  r"""Filters for Git merge queue events.

  Fields:
    targetBranch: Optional. Filter by the target branch of the merge queue.
  """

  targetBranch = _messages.MessageField('BranchFilter', 1)
...
class ParameterDefinition(_messages.Message):
  r"""Definition of a Pipeline input parameter.

  Fields:
    defaultValue: Optional. The default value if not provided at runtime.
      Omission of this parameter indicates the parameter is required.
    description: Optional. A description of the parameter for UI
      documentation.
  """
...
class PathFilter(_messages.Message):
  r"""Specifies file path filtering.

  Fields:
    excludedGlobs: Optional. Glob patterns for paths to exclude.
    includedGlobs: Optional. Glob patterns for paths to include.
  """

  excludedGlobs = _messages.StringField(1, repeated=True)
  includedGlobs = _messages.StringField(2, repeated=True)


class Pipeline(_messages.Message):
  r"""Message describing Pipeline object.

  Messages:
    AnnotationsValue: Optional. Annotations for the Pipeline.
    LabelsValue: Optional. User-defined labels for the Pipeline.

  Fields:
    annotations: Optional. Annotations for the Pipeline.
    config: Required. The declarative configuration for the pipeline, which
      includes the stages (DAG), remote configuration, timeouts, and
      parameters.
    createTime: Output only. Time when the Pipeline was created.
    deleteTime: Output only. Time when the Pipeline was deleted.
    description: Optional. A user-provided description of the Pipeline.
    displayName: Optional. User-provided display name for the Pipeline.
    etag: Optional. The checksum of the Pipeline resource.
    labels: Optional. User-defined labels for the Pipeline.
    name: Identifier. The name of the Pipeline. Format:
      `projects/{project}/locations/{location}/pipelines/{pipeline}`
    reconciling: Output only. A boolean that indicates whether the Pipeline is
      currently being reconciled.
    serviceAccount: Required. The SA used for Pipeline orchestration
      operations (e.g., resolving remote configs, evaluating CEL). This SA
      must have permission to act as the execution SA.
    uid: Output only. System-assigned unique identifier for the Pipeline.
      Differentiates between a pipeline that is recreated with the same name.
    updateTime: Output only. Time when the Pipeline was last updated.
  """

  @encoding.MapUnrecognizedFields('additionalProperties')
  class AnnotationsValue(_messages.Message):
    r"""Optional. Annotations for the Pipeline.
  ...
  class LabelsValue(_messages.Message):
    r"""Optional. User-defined labels for the Pipeline.
  ...
  config = _messages.MessageField('PipelineConfig', 2)
  createTime = _messages.StringField(3)
...
class PipelineConfig(_messages.Message):
  r"""PipelineConfig represents the core execution definition of a Pipeline.
  This message is snapshotted into the PipelineRun at the start of execution.

  Messages:
    ParamsValue: Optional. The runtime parameter assignments passed into this
      configuration.

  Fields:
    buildDefaults: Optional. Default settings applied to all builds in the
      pipeline.
    metadata: Optional. The static metadata and parameter schema for this
      pipeline.
    params: Optional. The runtime parameter assignments passed into this
      configuration.
    pubsubTopic: Optional. Pub/Sub topic to publish PipelineRun status updates
      to. Build updates will also be published to this topic.
    remoteConfig: Optional. If set, this Pipeline is defined in a remote
      repository. The URI specifies where to fetch the pipeline definition
      (e.g., from Developer Connect).
    stages: Optional. Required (unless remote_config is set). The stages to be
      executed. These stages define the Directed Acyclic Graph (DAG) of the
      workflow. A maximum of 100 stages is supported.
    timeout: Optional. The maximum time a PipelineRun can take before timing
      out. If not set, a default system timeout is applied.
  """

  @encoding.MapUnrecognizedFields('additionalProperties')
  class ParamsValue(_messages.Message):
    r"""Optional. The runtime parameter assignments passed into this
    configuration.
  ...
  buildDefaults = _messages.MessageField('BuildDefaults', 1)
  metadata = _messages.MessageField('PipelineMetadata', 2)
  params = _messages.MessageField('ParamsValue', 3)
  pubsubTopic = _messages.StringField(4)
  remoteConfig = _messages.StringField(5)
  stages = _messages.MessageField('Stage', 6, repeated=True)
  timeout = _messages.StringField(7)


class PipelineMetadata(_messages.Message):
  r"""PipelineMetadata encapsulates descriptive information and schemas for a
  remote or inline pipeline configuration.

  Messages:
    ParamsValue: Optional. Input parameters declaration for the Pipeline. This
      defines the schema (type, default values) expected. If the user provides
      a parameter during PipelineRun that is not specified here or in
      PipelineConfig.params, the system will still accept it.

  Fields:
    description: Optional. A description of the Pipeline's functional
      template.
    params: Optional. Input parameters declaration for the Pipeline. This
      defines the schema (type, default values) expected. If the user provides
      a parameter during PipelineRun that is not specified here or in
      PipelineConfig.params, the system will still accept it.
  """
  ...
  @encoding.MapUnrecognizedFields('additionalProperties')
  class ParamsValue(_messages.Message):
    r"""Optional. Input parameters declaration for the Pipeline. This defines
    the schema (type, default values) expected. If the user provides a
    parameter during PipelineRun that is not specified here or in
    PipelineConfig.params, the system will still accept it.
    ...
    class AdditionalProperty(_messages.Message):
      ...
      Fields:
        key: Name of the additional property.
        value: A ParameterDefinition attribute.
      """

      key = _messages.StringField(1)
      value = _messages.MessageField('ParameterDefinition', 2)
...
class PipelineRun(_messages.Message):
  r"""A run of the pipeline.

  Enums:
    StateValueValuesEnum: Output only. Current state of the PipelineRun.

  Messages:
    EvaluatedParamsValue: Output only. The final evaluated parameters after
      Phase 1 CEL evaluation.
    InputParamsValue: Optional. Input overrides provided by the user or
      trigger at run creation time.
    LabelsValue: Optional. User-defined labels for the PipelineRun.

  Fields:
    configSnapshot: Output only. A point-in-time copy of the PipelineConfig
      when this run was created. Serves as the single source of truth for the
      DAG and stage definitions.
    createTime: Output only. Time when the PipelineRun was created.
    evaluatedParams: Output only. The final evaluated parameters after Phase 1
      CEL evaluation.
    failureInfo: Output only. Detailed error information if state is FAILED.
    finishTime: Output only. Time when the PipelineRun reached a terminal
      state.
    inputParams: Optional. Input overrides provided by the user or trigger at
      run creation time.
    labels: Optional. User-defined labels for the PipelineRun.
    name: Output only. Identifier. The resource name of the PipelineRun.
      Format:
      `projects/{project}/locations/{location}/pipelineRuns/{pipeline_run}`
    pipeline: Optional. Output only. Immutable. The authoritative reference to
      the parent Pipeline. Format:
      `projects/{project}/locations/{location}/pipelines/{pipeline}`
    pipelineInfo: Output only. If the pipeline used remote_config, this
    ...
    state: Output only. Current state of the PipelineRun.
    trigger: Output only. The resource name of the trigger that initiated this
      run, if applicable.
    uid: Output only. System-assigned unique identifier for the PipelineRun.
    updateTime: Output only. Time when the PipelineRun was last updated.
  """

  class StateValueValuesEnum(_messages.Enum):
    r"""Output only. Current state of the PipelineRun.
  ...
  @encoding.MapUnrecognizedFields('additionalProperties')
  class EvaluatedParamsValue(_messages.Message):
    r"""Output only. The final evaluated parameters after Phase 1 CEL
    evaluation.
  ...
  @encoding.MapUnrecognizedFields('additionalProperties')
  class InputParamsValue(_messages.Message):
    r"""Optional. Input overrides provided by the user or trigger at run
    creation time.

    Messages:
      AdditionalProperty: An additional property for a InputParamsValue
        object.

    Fields:
      additionalProperties: Additional properties of type InputParamsValue
    """

    class AdditionalProperty(_messages.Message):
      r"""An additional property for a InputParamsValue object.
  ...
  @encoding.MapUnrecognizedFields('additionalProperties')
  class LabelsValue(_messages.Message):
    r"""Optional. User-defined labels for the PipelineRun.
  ...
  configSnapshot = _messages.MessageField('PipelineConfig', 1)
  createTime = _messages.StringField(2)
  evaluatedParams = _messages.MessageField('EvaluatedParamsValue', 3)
  failureInfo = _messages.MessageField('PipelineRunFailureInfo', 4)
  finishTime = _messages.StringField(5)
  inputParams = _messages.MessageField('InputParamsValue', 6)
...
class PipelineRunFailureInfo(_messages.Message):
  ...
  Enums:
    TypeValueValuesEnum: Output only. The classification of the failure.

  Messages:
    MetadataValue: Output only. Arbitrary key-value pairs for programmatic
      debugging.

  Fields:
    detail: Output only. A human-readable diagnostic message explaining the
      failure.
    metadata: Output only. Arbitrary key-value pairs for programmatic
      debugging.
    type: Output only. The classification of the failure.
  """

  class TypeValueValuesEnum(_messages.Enum):
    r"""Output only. The classification of the failure.

    Values:
      FAILURE_TYPE_UNSPECIFIED: Unspecified failure type.
      RESOLUTION_ERROR: Error resolving remote configuration.
      PERMISSION_ERROR: Permission denied during execution.
      CONFIG_ERROR: Invalid pipeline configuration.
      BUILD_FAILURE: Underlying build failed.
  ...
  @encoding.MapUnrecognizedFields('additionalProperties')
  class MetadataValue(_messages.Message):
    r"""Output only. Arbitrary key-value pairs for programmatic debugging.
...
class PipelineRunRef(_messages.Message):
  r"""Reference to the originating PipelineRun and Stage.

  Fields:
    pipelineRun: Output only. The resource name of the PipelineRun. Format:
      `projects/{project}/locations/{location}/pipelineRuns/{pipeline_run}`
    stageId: Output only. The unique ID of the stage within the pipeline
      configuration.
  """

  pipelineRun = _messages.StringField(1)
  stageId = _messages.StringField(2)


class PipelineRunStage(_messages.Message):
  r"""A stage in the PipelineRun.

  Fields:
    build: State for a stage configured as a standard Cloud Build.
    displayName: Output only. The display name of the stage.
    failureInfo: Output only. Detailed error information if this stage failed.
    id: Output only. The unique identifier of the stage from the Pipeline
      config.
  """

  build = _messages.MessageField('BuildStage', 1)
  displayName = _messages.StringField(2)
  failureInfo = _messages.MessageField('PipelineRunFailureInfo', 3)
  id = _messages.StringField(4)


class PipelineTrigger(_messages.Message):
  r"""Represents a PipelineTrigger, a resource that defines when and how its
  parent Pipeline should be executed.

  Messages:
    AnnotationsValue: Optional. User-defined annotations for the
      PipelineTrigger.
    LabelsValue: Optional. User-defined labels for the PipelineTrigger.
    ParamsValue: Optional. Parameters to pass to the PipelineRun. Values can
      be static strings or CEL expressions evaluated against the event payload
      (e.g., "${{ scm.sha }}").

  Fields:
    annotations: Optional. User-defined annotations for the PipelineTrigger.
    branchPush: Optional. Filter for branch pushes.
    createTime: Output only. Creation time of the PipelineTrigger.
    deleteTime: Output only. Time at which the PipelineTrigger was deleted.
    displayName: Optional. Human-readable name for the PipelineTrigger.
    etag: The etag of the PipelineTrigger.
    gitRepositoryLink: Optional. Developer Connect Git Repository Link Format:
      projects/{project}/locations/{location}/connections/{connection}/gitRepo
      sitoryLinks/{gitRepositoryLink}
    labels: Optional. User-defined labels for the PipelineTrigger.
    mergeQueue: Optional. Filter for merge queue events.
    name: Identifier. The resource name of the trigger. Format: `projects/{pro
      ject}/locations/{location}/pipelines/{pipeline}/triggers/{trigger}`
    options: Optional. Generic options for the pipeline trigger.
    params: Optional. Parameters to pass to the PipelineRun. Values can be
      static strings or CEL expressions evaluated against the event payload
      (e.g., "${{ scm.sha }}").
    pubSub: Optional. Filter for Pub/Sub messages.
    pullRequest: Optional. Filter for pull requests.
    reconciling: Output only. Indicates whether the PipelineTrigger is
      currently being reconciled.
    repositoryUrl: Optional. Developer Connect Repository URL (e.g.,
      https://github.com/author/repo) This must correspond to a resource
      linked via Developer Connect.
    tagPush: Optional. Filter for tag pushes.
    topic: Optional. Pub/Sub Topic Format: projects/{project}/topics/{topicId}
    uid: Output only. Unique identifier of the PipelineTrigger.
    updateTime: Output only. Update time of the PipelineTrigger.
    webhook: Optional. Filter for webhooks.
    webhookSecretVersion: Optional. Secret Manager Secret Version (for
      Webhooks) Format:
      projects/{project}/secrets/{secretId}/versions/{versionId}
  """

  @encoding.MapUnrecognizedFields('additionalProperties')
  class AnnotationsValue(_messages.Message):
    r"""Optional. User-defined annotations for the PipelineTrigger.
  ...
  @encoding.MapUnrecognizedFields('additionalProperties')
  class LabelsValue(_messages.Message):
    r"""Optional. User-defined labels for the PipelineTrigger.
  ...
  @encoding.MapUnrecognizedFields('additionalProperties')
  class ParamsValue(_messages.Message):
    r"""Optional. Parameters to pass to the PipelineRun. Values can be static
    strings or CEL expressions evaluated against the event payload (e.g., "${{
    scm.sha }}").
  ...
  annotations = _messages.MessageField('AnnotationsValue', 1)
  branchPush = _messages.MessageField('PipelineTriggerPushFilter', 2)
  createTime = _messages.StringField(3)
  deleteTime = _messages.StringField(4)
  displayName = _messages.StringField(5)
  etag = _messages.StringField(6)
  gitRepositoryLink = _messages.StringField(7)
  labels = _messages.MessageField('LabelsValue', 8)
  mergeQueue = _messages.MessageField('MergeQueueFilter', 9)
  name = _messages.StringField(10)
  options = _messages.MessageField('TriggerOptions', 11)
  params = _messages.MessageField('ParamsValue', 12)
  pubSub = _messages.MessageField('PubSubFilter', 13)
  pullRequest = _messages.MessageField('PipelineTriggerPullRequestFilter', 14)
  reconciling = _messages.BooleanField(15)
  repositoryUrl = _messages.StringField(16)
  tagPush = _messages.MessageField('PipelineTriggerPushFilter', 17)
  topic = _messages.StringField(18)
  uid = _messages.StringField(19)
  updateTime = _messages.StringField(20)
  webhook = _messages.MessageField('WebhookFilter', 21)
  webhookSecretVersion = _messages.StringField(22)


class PipelineTriggerPullRequestFilter(_messages.Message):
  r"""Filters for Git pull request events.

  Fields:
    celFilters: Optional. Advanced filtering using Common Expression Language
      (CEL).
    pathFilter: Optional. Filter based on file paths modified in the PR.
    targetBranch: Optional. Filter by the target branch of the PR.
  """

  celFilters = _messages.MessageField('CelFilter', 1, repeated=True)
  pathFilter = _messages.MessageField('PathFilter', 2)
  targetBranch = _messages.MessageField('BranchFilter', 3)


class PipelineTriggerPushFilter(_messages.Message):
  r"""Filters for Git push events (branches or tags).

  Fields:
    celFilters: Optional. Advanced filtering using Common Expression Language
      (CEL).
    gitRef: Optional. Filter by Git ref (branch or tag).
    pathFilter: Optional. Filter based on file paths modified in the push.
  """

  celFilters = _messages.MessageField('CelFilter', 1, repeated=True)
  gitRef = _messages.MessageField('GitRefFilter', 2)
  pathFilter = _messages.MessageField('PathFilter', 3)
...
class PubSubFilter(_messages.Message):
  r"""Filters for Pub/Sub messages.

  Fields:
    celFilters: Optional. Advanced filtering using Common Expression Language
      (CEL) on the Pub/Sub message attributes and data.
  """
...
class ReceivePipelineTriggerWebhookResponse(_messages.Message):
  r"""Response for receiving webhooks posted to a PipelineTrigger."""
...
class SnapshotInfo(_messages.Message):
  r"""SnapshotInfo records the provenance of remote configurations.

  Fields:
    resolveTime: Output only. The time when the resolution occurred.
    sha: Output only. The specific commit SHA or revision resolved for this
      execution.
    uri: Output only. The URI of the remote repository (e.g. Developer Connect
      link).
  """

  resolveTime = _messages.StringField(1)
...
class Stage(_messages.Message):
  r"""A Stage represents a single unit of execution (a Cloud Build) within the
  Pipeline.

  Fields:
    build: Defines the stage as a standard Cloud Build. This reuses the
      existing `Build` message but interpreted as a template.
    dependsOn: Optional. List of Stage IDs that this Stage depends on. This
      defines the DAG structure.
    displayName: Optional. A display name for the stage (e.g., "Build Backend
      Service"). This accepts free-form text.
    id: Required. Unique identifier for the stage (e.g., "build-backend").
      This is used as the key for the `depends_on` relationship. A stage will
      only transition from PENDING to RUNNING once all stage IDs listed in its
      depends_on array have reached a terminal SUCCEEDED state. This must be
      alphanumeric/hyphens with no spaces.
  """

  build = _messages.MessageField('Build', 1)
  dependsOn = _messages.StringField(2, repeated=True)
  displayName = _messages.StringField(3)
  id = _messages.StringField(4)
...
class TriggerOptions(_messages.Message):
  r"""Options for PipelineTrigger.

  Enums:
    LogLevelValueValuesEnum: Optional. Logging verbosity level for platform
      logs.

  Fields:
    disableSkipNotification: Optional. Disables the sending of neutral/skipped
      statuses back to SCMs.
    disabled: Optional. Disables the trigger.
    logLevel: Optional. Logging verbosity level for platform logs.
  """

  class LogLevelValueValuesEnum(_messages.Enum):
    r"""Optional. Logging verbosity level for platform logs.

    Values:
      LOG_LEVEL_UNSPECIFIED: Default level: logs all evaluations (invocations,
        errors, and skips).
      NONE: Disables platform logging completely for this trigger.
      ERRORS_ONLY: Logs only when a trigger encounters a setup or evaluation
        error (FAILED / FAILED_PRECONDITION).
      ALL: Verbose: logs all events including SKIPPED filter diagnostics and
        invocations.
    """
  ...
  disableSkipNotification = _messages.BooleanField(1)
...
class WebhookFilter(_messages.Message):
  r"""Filters for generic webhook events.

  Fields:
    celFilters: Optional. Advanced filtering using Common Expression Language
      (CEL) on the webhook payload and headers.
  """
