...
class BigQueryOptions(_messages.Message):
  ...
  Fields:
    usePartitionedTables: Optional. Whether to use BigQuery's partition tables
      ...
      (https://docs.cloud.google.com/bigquery/docs/querying-partitioned-
      tables) has to be used instead. In both cases, tables are sharded based
      on UTC timezone.
...
class CmekSettings(_messages.Message):
  ...
  Logging (https://docs.cloud.google.com/logging/docs/routing/managed-
  ...
  Fields:
    kmsKeyName: Optional. The resource name for the configured Cloud KMS
      ...
      for the Log Router, set this field to an empty string.See Configure CMEK
      for Cloud Logging
...
class DefaultSinkConfig(_messages.Message):
  ...
  Fields:
    ...
    filter: Optional. An advanced logs filter
      (https://docs.cloud.google.com/logging/docs/view/building-
      queries#queries-by-expression). The only exported log entries are those
...
class FieldSource(_messages.Message):
  r"""A source that can be used to represent a "field of data" within various
  ...
  Fields:
    ...
    field: The fully qualified, dot-delimited path to the selected atomic
      ...
      actions like drill-down or projection.The path components should match
      the exact field names or keys as they appear in the underlying data
      schema. For JSON fields, this means respecting the original casing
      (e.g., camelCase or snake_case as present in the JSON).To reference
      field names containing special characters (e.g., hyphens, spaces),
      enclose the individual path segment in backticks (`).Examples: *
      json_payload.labels.message * json_payload.request_id *
...
class LogEntry(_messages.Message):
  ...
  Messages:
    ...
    OtelValue: Optional. The structured OpenTelemetry protocol payload.
      Contains the OpenTelemetry Resource, Instrumentation Scope, and Entities
      attributes for this log as they are defined in the OTLP specification,
      and any other fields that do not have a direct analog in the LogEntry.
  ...
  Fields:
    ...
    errorGroups: Output only. The Error Reporting
      ...
      error group creation.This field is populated only when log entries are
      stored in Cloud Logging storage (Logs Explorer and Observability
      Analytics). It is not available for use in log sink filters, log-based
      metrics, or log-based alerts, and it is excluded from log exports (Cloud
      Storage, BigQuery, and Pub/Sub).For more information, see View error
    ...
    otel: Optional. The structured OpenTelemetry protocol payload. Contains
      the OpenTelemetry Resource, Instrumentation Scope, and Entities
      attributes for this log as they are defined in the OTLP specification,
      and any other fields that do not have a direct analog in the LogEntry.
      See https://opentelemetry.io/docs/specs/otel/logs/data-model/
    ...
    spanId: Optional. The ID of the Cloud Trace
      (https://docs.cloud.google.com/trace/docs) span associated with the
      current operation in which the log is being written.A Span (https://docs
      .cloud.google.com/trace/docs/reference/v2/rest/v2/projects.traces/batchW
      rite#Span) represents a single operation within a trace. Whereas a trace
      may involve multiple different microservices running on multiple
      different machines, a span generally corresponds to a single logical
      operation being performed in a single instance of a microservice on one
      specific machine. Spans are the nodes within the tree that is a
      ...
      expected to be a 16-character, hexadecimal encoding of an 8-byte array
      and should not be zero. It should be unique within the trace and should,
      ideally, be generated in a manner that is uniformly random.Example
      values: 000000000000004a 7a2190356c3fc94b 0000f00300090021
      d39223e101960076
    ...
    timestamp: Optional. The time the event described by the log entry
      ...
      be omitted when the timestamp is displayed.
    trace: Optional. The trace ID being written to Cloud Trace
      ...
      supported, but using this format is not recommended. An example trace
      REST resource name is similar to "projects/my-trace-
      project/traces/12345".The trace field provides the link between logs and
      traces. By using this field, you can navigate from a log entry to a
      trace.
  ...
  @encoding.MapUnrecognizedFields('additionalProperties')
  class OtelValue(_messages.Message):
    r"""Optional. The structured OpenTelemetry protocol payload. Contains the
    ...
    Messages:
      AdditionalProperty: An additional property for a OtelValue object.
    ...
    class AdditionalProperty(_messages.Message):
      r"""An additional property for a OtelValue object.
  ...
  otel = _messages.MessageField('OtelValue', 12)
  protoPayload = _messages.MessageField('ProtoPayloadValue', 13)
  receiveTimestamp = _messages.StringField(14)
  resource = _messages.MessageField('MonitoredResource', 15)
  severity = _messages.EnumField('SeverityValueValuesEnum', 16)
  sourceLocation = _messages.MessageField('LogEntrySourceLocation', 17)
  spanId = _messages.StringField(18)
  split = _messages.MessageField('LogSplit', 19)
  textPayload = _messages.StringField(20)
  timestamp = _messages.StringField(21)
  trace = _messages.StringField(22)
  traceSampled = _messages.BooleanField(23)
...
class LogErrorGroup(_messages.Message):
  ...
  Fields:
    id: The id is a unique identifier for a particular error group; it is the
      ...
      COShysOX0r_51QE.This field can be used to search for log entries
      belonging to a specific error group in Logs Explorer (e.g.,
      error_groups.id = "ID") or Observability Analytics.The id is derived
      from key parts of the error-log content and is treated as Service Data.
      For information about how Service Data is handled, see Google Cloud
...
class LogExclusion(_messages.Message):
  ...
  Fields:
    ...
    filter: Required. An advanced logs filter
      (https://docs.cloud.google.com/logging/docs/view/building-
      queries#queries-by-expression) that matches the log entries to be
      excluded. By using the sample function
      (https://docs.cloud.google.com/logging/docs/view/logging-query-
      language#sample), you can exclude less than 100% of the matching log
      entries.For example, the following query matches 99% of low-severity log
      entries from Google Cloud Storage buckets:resource.type=gcs_bucket
...
class LoggingBillingAccountsSinksCreateRequest(_messages.Message):
  ...
  Fields:
    ...
    uniqueWriterIdentity: Optional. Determines the kind of IAM identity
      ...
      be a service agent (https://docs.cloud.google.com/iam/docs/service-
      account-types#service-agents) used by the sinks with the same parent.
...
class LoggingBillingAccountsSinksPatchRequest(_messages.Message):
  ...
  Fields:
    ...
    uniqueWriterIdentity: Optional. See sinks.create for a description of this
      ...
      (https://docs.cloud.google.com/iam/docs/service-account-types#service-
      agents) owned by Cloud Logging. It is an error if the old value is true
...
class LoggingQuery(_messages.Message):
  ...
  Fields:
    filter: Required. An advanced query using the Logging Query Language
      (https://docs.cloud.google.com/logging/docs/view/logging-query-
      language). The maximum length of the filter is 20000 characters.
...
class OpsAnalyticsQuery(_messages.Message):
  r"""Describes a query that can be run in Log Analytics.

  Fields:
    queryBuilder: Optional. A query builder configuration used in Log
      Analytics.If both query_builder and sql_query_text fields are set, then
      the sql_query_text will be used, if its non-empty. At least one of the
    ...
    sqlQueryText: Optional. A Log Analytics SQL query in text format.If both
      ...
      will be used, if its non-empty. At least one of the two fields must be
      set.
  """
...
class ProjectedField(_messages.Message):
  ...
  virtualField = _messages.MessageField('VirtualField', 8)
...
class QueryDataResponse(_messages.Message):
  ...
  Fields:
    queryStepHandles: URL-safe handles to each of the query steps described in
      the request, excluding those for which the output_not_required flag was
      set. These may be passed to ReadQueryResults or used in a
...
class QueryDataSyncResponse(_messages.Message):
  ...
  Fields:
    nextPageToken: A URL-safe token that can be sent as page_token to retrieve
    ...
    queryStepHandle: An opaque, URL-safe string that can be used as a
      reference to this query result. This reference can be used in the
      ReadQueryResults query to fetch this result up to 24 hours in the
      future.This is populated if the query cannot be completed synchronously
      within the deadline of the QueryDataSync call, or if there are
...
class QueryResults(_messages.Message):
  ...
  Fields:
    ...
    restrictionConflicts: Conflicts between the query and the restrictions
      that were requested. Any restrictions present here were ignored when
      executing the query.
    resultReference: An opaque, URL-safe string that can be used as a
      reference to this query result. This result reference can be used in the
      ReadQueryResults query to fetch this result up to 24 hours in the
      future.
    rows: Query result rows. The number of rows returned depends upon the page
      size requested. To get any additional rows, you can call
...
class ReadQueryResultsRequest(_messages.Message):
  ...
  Fields:
    ...
    queryStepHandle: Required. A query step handle returned by QueryData or
      QueryDataSync.
...
class RecentQuery(_messages.Message):
  ...
  Fields:
    ...
    name: Output only. Resource name of the recent query.In the format:
      "projects/[PROJECT_ID]/locations/[LOCATION_ID]/recentQueries/[QUERY_ID]"
      For a list of supported locations, see Supported Regions
      (https://docs.cloud.google.com/logging/docs/region-support)The QUERY_ID
      is a system generated alphanumeric ID.
...
class Settings(_messages.Message):
  ...
  Fields:
    ...
    kmsServiceAccountId: Output only. The service account that will be used by
      ...
      GetSettings to obtain the service account ID.See Configure CMEK for
      Cloud Logging
      (https://docs.cloud.google.com/logging/docs/routing/managed-encryption)
      for more information.
...
class SortOrderParameter(_messages.Message):
  ...
  fieldSource = _messages.MessageField('FieldSource', 1)
...
class SummaryField(_messages.Message):
  ...
  (https://docs.cloud.google.com/logging/docs/view/logs-explorer-
  interface#preferences) for a query in the Logs Explorer.
...
class VirtualField(_messages.Message):
  r"""A virtual field is a field that is not physically present in the
  underlying data schema, but is created through specific operations within
  the query builder model based on other fields in the schema.

  Enums:
    VirtualFieldTypeValueValuesEnum: Required. The type of the virtual field.

  Fields:
    underlyingFieldSources: The field sources that will be used to create the
      virtual field, based on the semantics of the virtual field type.The
      field sources must follow these rules, based on the virtual field type:
      - For VIRTUAL_FIELD_TYPE_UNSPECIFIED, this field must be empty. - For
      COALESCE, this field must be non-empty and include a minimum of two
      ...
      transformed and aggregated in any way, except for casting. The type of
      all the underlying field sources must be equivalent so that picking one
      of them would result in the same value type.
    virtualFieldType: Required. The type of the virtual field.
  """

  class VirtualFieldTypeValueValuesEnum(_messages.Enum):
    r"""Required. The type of the virtual field.
    ...
    Values:
      ...
      COALESCE: Creates a virtual field by selecting the first non-null value
        from the list of fields specified in underlying_field_sources, similar
        to a COALESCE function in SQL.
    """
    VIRTUAL_FIELD_TYPE_UNSPECIFIED = 0
    COALESCE = 1

  underlyingFieldSources = _messages.MessageField('FieldSource', 1, repeated=True)
  virtualFieldType = _messages.EnumField('VirtualFieldTypeValueValuesEnum', 2)
...
class WriteLogEntriesRequest(_messages.Message):
  ...
  Fields:
    ...
    entries: Required. The log entries to send to Logging. The order of log
      ...
      (https://docs.cloud.google.com/logging/quotas) in the past or more than
      ...
      (https://docs.cloud.google.com/logging/quotas) for calls to
