...
class NotebooksProjectsLocationsInstancesPatchRequest(_messages.Message):
  ...
  Fields:
    ...
    updateMask: Required. Mask used to update an instance. Updatable fields: *
      ...
      *disable* the feature (set the field to `true`). Requests that set
      either field back to `false` (re-enabling the external IP or proxy
      access) are rejected with `INVALID_ARGUMENT`.
  """
