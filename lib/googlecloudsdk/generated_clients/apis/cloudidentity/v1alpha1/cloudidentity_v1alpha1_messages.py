...
class BrowserAttributes(_messages.Message):
  r"""Contains information about Browser Profiles reported by Clients on the
  device.
...
class BrowserInfo(_messages.Message):
  ...
  Fields:
    ...
    passwordProtectionWarningTrigger: Current state of password protection
      trigger.
    policies: Output only. Chrome policies information for the browser.
...
class ChromePolicy(_messages.Message):
  r"""Chrome policy information.

  Enums:
    ScopeValueValuesEnum: Output only. The scope of the policy.
    SourceValueValuesEnum: Output only. The source of the policy.

  Fields:
    conflicts: Output only. The conflicts with other policies with same name.
    name: Output only. The name of the policy.
    scope: Output only. The scope of the policy.
    source: Output only. The source of the policy.
    value: Output only. The value of the policy.
  """

  class ScopeValueValuesEnum(_messages.Enum):
    r"""Output only. The scope of the policy.

    Values:
      SCOPE_UNKNOWN: Policy scope is unknown.
      USER: User policies apply to current Session/Profile if it's cloud
        policy. Or apply to current OS user on Windows.
      MACHINE: Machine policies apply to any users of the current machine.
    """
    SCOPE_UNKNOWN = 0
    USER = 1
    MACHINE = 2
  ...
  class SourceValueValuesEnum(_messages.Enum):
    r"""Output only. The source of the policy.

    Values:
      SOURCE_UNKNOWN: Policy source is unknown.
      ENTERPRISE_DEFAULT: A policy is set by Chrome when it's running in an
        enterprise environment.
      CLOUD: A policy is set by Google's cloud management tool.
      ACTIVE_DIRECTORY: A policy is set by active directory on ChromeOS.
      DEVICE_LOCAL_ACCOUNT_OVERRIDE_DEPRECATED: Deprecated: A policy is
        overridden by ChromeOS if it's running in a public session or kiosk
        mode.
      PLATFORM: A policy is set by OS built-in tool on desktop.
      PRIORITY_CLOUD_DEPRECATED: Deprecated: A policy is set by Google's cloud
        management tool but has higher priority.
      MERGED: A policy is set by multiple sources and value has been merged.
      COMMAND_LINE: A policy is set by command line switch for testing
        purpose.
      CLOUD_FROM_ASH: A policy is set by Google's cloud management tool in Ash
        and piped to Lacros.
      RESTRICTED_MANAGED_GUEST_SESSION_OVERRIDE: A policy that is set by the
...
class CloudidentityGroupsSearchRequest(_messages.Message):
  ...
  Fields:
    ...
    query: Required. The search query. * Must be specified in [Common
      Expression Language](https://opensource.google/projects/cel). See [CEL
      Introduction](https://github.com/google/cel-
      ...
      `display_name == 'dev'` Examples: * Search for all discussion forums
      under a customer: `parent == 'customers/C046psxkn' &&
      'cloudidentity.googleapis.com/groups.discussion_forum' in labels` *
      Search for groups with key starting with 'sales': `parent ==
...
class ExternalId(_messages.Message):
  r"""A secondary identifier for an entity in the Cloud Identity Groups API.
  ...
  Fields:
    id: Required. The unique identifier assigned by the external identity
      provider. The API does not enforce uniqueness of the Ids, but it is best
      practice for the client to ensure that the Ids are unique.
...
class Group(_messages.Message):
  ...
  of entities, where each entity is either a user or another group. Next id:
  13
...
class PolicyConflict(_messages.Message):
  r"""A policy conflict indicates a conflict between different policy sources.
