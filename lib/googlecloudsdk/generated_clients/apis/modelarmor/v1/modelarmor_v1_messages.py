...
class McpServerFloorSetting(_messages.Message):
  ...
  Fields:
    apis: Optional. List of MCP servers for which the MCP floor setting is
...
class MultiLanguageDetectionMetadata(_messages.Message):
  ...
  Fields:
    ...
    sourceLanguage: Optional. Optional Source language of the user prompt. If
      ...
      source language will be automatically detected. When a source language
      is provided, Model Armor uses it to sanitize the input. In that case the
      system does not perform auto-detection and relies solely on the
      specified language. This string field accepts a language code from the
      ISO-639 standard. For a list of languages supported by Model Armor, see
      [Model Armor supported languages] (https://cloud.google.com/security-
      command-center/docs/model-armor-overview#languages-supported). For a
      comprehensive list of language codes, see
...
class SdpFindingLocation(_messages.Message):
  ...
  Fields:
    byteRange: Zero-based byte offsets delimiting the finding. These are
      ...
      of the content. Note: Omitted if content is an image.
