...
class DataprocProjectsRegionsJobsListRequest(_messages.Message):
  ...
  Fields:
    ...
    filter: Optional. A filter constraining the jobs to list. Filters are
      ...
      can be either ACTIVE or NON_ACTIVE. Allows insertTime to be a timestamp
      in RFC 3339 format in double quotes, such as 2025-01-01T00:00:00Z. Only
      the logical AND operator is supported; space-separated items are treated
      as having an implicit AND operator.Example filter:status.state = ACTIVE
      AND labels.env = staging AND labels.starred = * AND insertTime <=
