...
class Parameter(_messages.Message):
  ...
  Messages:
    ...
    TagsValue: Optional. Input only. Immutable. Tag keys and tag values that
      are bound to this Parameter. You must represent each item in the map as:
      `"" : ""`. For example, a single resource can have the following tags:
      ``` "123/environment": "production", "123/costCenter": "marketing", ```
      Tags are used to organize and group resources. Tags can be used to
...
class ParameterVersion(_messages.Message):
  ...
  checksumSource = _messages.EnumField('ChecksumSourceValueValuesEnum', 1)
  createTime = _messages.StringField(2)
  disabled = _messages.BooleanField(3)
  kmsKeyVersion = _messages.StringField(4)
  name = _messages.StringField(5)
  payload = _messages.MessageField('ParameterVersionPayload', 6)
