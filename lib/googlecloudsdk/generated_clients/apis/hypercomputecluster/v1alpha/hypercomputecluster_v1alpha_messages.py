...
class BootDisk(_messages.Message):
  ...
  Fields:
    ...
    image: Optional. Source image family used to create this disk. Must be a
      [supported image family](https://docs.cloud.google.com/cluster-
      director/docs/compute#os-images) for the VM instance's machine type. If
      no family is specified, the system will select a supported image family
...
class Cluster(_messages.Message):
  ...
  Fields:
    ...
    description: Optional. A description for your cluster. You can use up to
      2,048 characters.
    healthChecks: Optional. Health checks on the cluster.
  ...
  healthChecks = _messages.MessageField('HealthChecks', 4)
...
class ComputeEngineNodeDetails(_messages.Message):
  ...
  Fields:
    ...
    instanceGroupManager: Output only. [Relative resource
      name](https://google.aip.dev/122) of the managed instance group, in the
...
class HealthChecks(_messages.Message):
  r"""Health check configuration for a cluster.

  Enums:
    DisabledNodeActionsValueListEntryValuesEnum:

  Fields:
    disabledNodeActions: Optional. List of actions that are disabled for
      built-in health check failures.
    enableStandardChecks: Optional. Whether built-in (Google-provided) health
      checks are enabled.
  """

  class DisabledNodeActionsValueListEntryValuesEnum(_messages.Enum):
    r"""DisabledNodeActionsValueListEntryValuesEnum enum type.
...
class HypercomputeclusterProjectsLocationsMachineLearningRunsProfilerSessionsPatchRequest(_messages.Message):
  r"""A HypercomputeclusterProjectsLocationsMachineLearningRunsProfilerSession
...
class Node(_messages.Message):
  ...
  class StateValueValuesEnum(_messages.Enum):
    ...
    Values:
      ...
      UNPROVISIONED: The node has no VM assigned to it.
    """
    STATE_UNSPECIFIED = 0
    CREATING = 1
    ACTIVE = 2
...
class SlurmNodeSet(_messages.Message):
  ...
  Enums:
    EnabledHealthChecksValueListEntryValuesEnum:
  ...
  Fields:
    ...
    enablePublicIps: Optional. Whether compute node instances should be
      ...
      addresses#externaladdresses).
    enabledHealthChecks: Output only. The built-in health checks effectively
      running on this nodeset.
  ...
  class EnabledHealthChecksValueListEntryValuesEnum(_messages.Enum):
    r"""EnabledHealthChecksValueListEntryValuesEnum enum type.

    Values:
      TYPE_UNSPECIFIED: Not set.
      DCGM: Health checks performed using NVIDIA DCGM. These checks run
        diagnostics to verify the health and proper functioning of NVIDIA GPU
        hardware.
    """
    TYPE_UNSPECIFIED = 0
    DCGM = 1
...
class StorageResource(_messages.Message):
  r"""Represents a form of persistent storage that you can mount onto compute
  resources in the cluster.

  Fields:
    bucket: Output only. A reference to a Google Cloud Storage bucket.
    ...
    lustre: Output only. A reference to a Managed Lustre instance. Populated
      if and only if the storage resource was configured to use Managed
      Lustre.
  """
