...
class OrgpolicyV2(base_api.BaseApiClient):
  ...
  class FoldersPoliciesService(base_api.BaseApiService):
    ...
    def Get(self, request, global_params=None):
      r"""Gets a policy on a resource. If no policy is set on the resource, `NOT_FOUND` is returned. The entity tag (ETag) can be used with `UpdatePolicy()` to update a policy during read-modify-write.
