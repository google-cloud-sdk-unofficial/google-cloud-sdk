...
def clone(
    ...
    **kwargs,
    ...
    if outstream is not None:
        ...
        # TODO(user): Capture logging output and stream to errstream
    ...
    with open_repo_closing(repo) as r:
        ...
        for p in paths:
            ...
            if path.is_symlink():
                ...
                if path.resolve().is_dir():
                    try:
                        ...
                        raise ValueError(
                            f"Path {path.resolve()} is not in the subpath of {repo_path}"
                        )
            else:
            ...
            try:
                ...
                raise ValueError(
                    f"Path {p} is not in the subpath of repository {repo_path}"
                ) from e
...
def submodule_add(
    ...
    with open_repo_closing(repo) as r:
        ...
        # TODO(user): Move this logic to dulwich.submodule
