...
try:
    ...
    class FallbackMerge3:
        """Fallback pure-Python three-way merge using difflib when merge3 is unavailable."""

        def __init__(self, base: list[bytes], a: list[bytes], b: list[bytes]) -> None:
            self.base = base
            self.a = a
            self.b = b
        ...
        def merge_groups(self):
            ...
            if self.base == self.b:
                yield ("a", self.a)
                return
            sm = difflib.SequenceMatcher(None, self.a, self.b)
            base_idx = 0
            for tag, i1, i2, j1, j2 in sm.get_opcodes():
                if tag == "equal":
                    a_part = self.a[i1:i2]
                    sm_base = difflib.SequenceMatcher(None, self.base[base_idx:], a_part)
                    match = sm_base.find_longest_match(
                        0, len(self.base) - base_idx, 0, len(a_part)
                    )
                    if match.size == len(a_part):
                        base_part = self.base[base_idx : base_idx + match.a]
                        base_idx = base_idx + match.a + match.size
                    else:
                        base_part = []
                    if base_part:
                        yield ("conflict", base_part, [], [])
                    yield ("same", a_part)
                else:
                    a_part = self.a[i1:i2]
                    b_part = self.b[j1:j2]
                    if self.base[base_idx : base_idx + len(a_part)] == a_part:
                        base_part = self.base[base_idx : base_idx + len(a_part)]
                        base_idx += len(a_part)
                    elif self.base[base_idx : base_idx + len(b_part)] == b_part:
                        base_part = self.base[base_idx : base_idx + len(b_part)]
                        base_idx += len(b_part)
                    else:
                        base_part = self.base[
                            base_idx : base_idx + max(len(a_part), len(b_part))
                        ]
                        base_idx += len(base_part)
                    yield ("conflict", base_part, a_part, b_part)
            if base_idx < len(self.base):
                yield ("conflict", self.base[base_idx:], [], [])

    merge3 = type("merge3", (), {"Merge3": FallbackMerge3})  # type: ignore
